...
public class ParagraphListInfo {
    ...
    public LineSegItem currentLineSeg() {
        if (currentPara.getLineSeg() == null || currentPara.getLineSeg().getLineSegItemList().size() <= lineSeqIndex) {
            return null;
        }
        return currentPara.getLineSeg().getLineSegItemList().get(lineSeqIndex);
    }

    public LineSegItem nextLineSeg() {

        if (currentPara.getLineSeg() == null || currentPara.getLineSeg().getLineSegItemList().size() <= lineSeqIndex + 1) {
            return null;
        }
        return currentPara.getLineSeg().getLineSegItemList().get(lineSeqIndex + 1);
    }
