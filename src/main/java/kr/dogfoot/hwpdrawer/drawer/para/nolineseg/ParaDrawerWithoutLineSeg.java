...
import kr.dogfoot.hwpdrawer.drawer.para.nolineseg.textflow.TextFlowCalculationResult;
import kr.dogfoot.hwpdrawer.drawer.para.nolineseg.textflow.TextFlowCalculator;
import kr.dogfoot.hwpdrawer.drawer.paralist.DistributionMultiColumnRearranger;
...
public class ParaDrawerWithoutLineSeg extends ParaDrawer {
    ...
    public ParaDrawerWithoutLineSeg(DrawingInput input, InterimOutput output) {
        super(input, output);
    ...
    private long endY() {
        ...
        return endY + input.paraShape().getBottomParaSpace() / 2;
    }
    ...
    private void resetCurrentTextArea() {
        ...
        currentTextArea.applyMargin(input.paraShape().getLeftMargin() / 2,
                0,
                input.paraShape().getRightMargin() / 2,
                0);
        if (firstLine) {
            if (input.paraShape().getIndent() > 0) {
                currentTextArea.left(currentTextArea.left() + input.paraShape().getIndent() / 2);
            }
            currentTextArea.top(currentTextArea.top() + input.paraShape().getTopParaSpace() / 2);
        } else {
            currentTextArea.left(currentTextArea.left() - input.paraShape().getIndent() / 2);
        }
    }
